...
public final class ObjectUtil {
  ...
  public static <F, S, R> R nullChecker(final F first, final S second,
      final Function<S, R> firstNull, final Function<F, R> secondNull,
  ...
  public static <T, F extends Comparable<F>, E extends Comparable<E>> int compareWithNullCheck(
    ...
    return ObjectUtil.compareWithNullCheck((f, s) -> {
      ...
      int ret = ObjectUtil.compareWithNullCheck(Comparable::compareTo,
          firstCompareFunction.apply(f), firstCompareFunction
              .apply(s));

      if (ret == 0) {

        ret = ObjectUtil.compareWithNullCheck(Comparable::compareTo, secondCompareFunction.apply(f),
            secondCompareFunction
                .apply(s));
  ...
  public static <T, F extends Comparable<F>, E extends Comparable<E>, G extends Comparable<G>> int compareWithNullCheck(
      final T first,
      final T second,
      final Function<T, F> firstCompareFunction,
      final Function<T, E> secondCompareFunction,
    ...
    final int ret = ObjectUtil.compareWithNullCheck(first, second, firstCompareFunction,
        secondCompareFunction);
  ...
  public static <T> int compareWithNullCheck(final BiFunction<T, T, Integer> compareFunction,
      final T first, final T second) {
  ...
  public static <F, S> Boolean equalsWithNullCheck(final BiFunction<F, S, Boolean> equalsFunction,
  ...
  @SafeVarargs
  private static <T, W> boolean _checkGetters(final T first, final T second,
      final Function<T, W>... getters) {

    for (final Function<T, W> getter : getters) {

      if (!ObjectUtil.equalsWithNullCheck(Object::equals, getter.apply(first),
          getter.apply(second))) {
        return false;
      }
  ...
  public static <T, W> boolean ifSameClass(final T first, final W second,
      final BiFunction<T, T, Boolean> furtherCompareFunction) {

    try {

      @SuppressWarnings({"unchecked"}) final T secondCasted = (T) second;

      return furtherCompareFunction != null ? furtherCompareFunction.apply(first, secondCasted)
          : false;
  ...
  public static <T extends IdHolder<?>, W> boolean isSameClassWithSameId(final T first,
      final W second) {
  ...
  public static <T> boolean equalsWithNullCheck(final T first, final T second) {
    ...
    return (first == second) || (ObjectUtil.nullChecker(first, second, f -> false, s -> false,
        () -> true,
  ...
  @SafeVarargs
  public static <T, W> boolean equalsWithNullCheck(final T first, final W second,
  ...
  public static <T, W> boolean equalsWithNullCheck(final BiFunction<T, W, Boolean> equalsFunction,
      final T first, final W second,
      final BiFunction<T, T, Boolean> extension) {

    return ObjectUtil.equalsWithNullCheck(equalsFunction, first, second) && extension.apply(first,
        (T) second);
  ...
  public static <T, W> boolean equalsWithNullCheck(final BiFunction<T, W, Boolean> equalsFunction,
    ...
    return ObjectUtil.equalsWithNullCheck(equalsFunction, first, second,
        (f, s) -> ObjectUtil._checkGetters(f, s, getters));

  }

  @SafeVarargs
  public static <T extends IdHolder<?>, W> boolean equalsWithNullCheck(final T first,
      final W second,
      final Function<T, W> mandatoryGetter,
      final Function<T, W>... getters) {
  ...
  public static <O extends Comparable<? super O>> O minMaxOrElse(@NotNull final O min,
      @NotNull final O max, @NotNull final O fallback) {
